...
export function ChewTab({ queryParams, view }: ChewTabProps) {
      ...
      <div className={cn(
        ...
        {[...Array(10)].map((_, i) => (
          <div key={i} className={cn(
            "bg-capy-tan/5 rounded-3xl animate-pulse border border-capy-tan/10",
            view === 'grid' ? "h-[320px]" : "h-14"
          )} />
        ))}
      </div>
    );
  ...
  if (error || !pairs) {
    return (
      <Card className="border-dashed py-12 text-center bg-white rounded-3xl shadow-none border-capy-tan/20">
        ...
        <AnimatePresence mode="popLayout">
          {pairs.length > 0 ? (
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="col-span-full">
              <Card className="border-dashed py-24 text-center bg-white rounded-3xl shadow-none border-capy-tan/20">
                <p className="text-capy-dark/30 font-medium text-xs uppercase tracking-widest">No matches found</p>
              </Card>
            </motion.div>
          )}
        </AnimatePresence>
...
function TokenCard({ token, view }: { token: ScoredPair, view: 'grid' | 'list' }) {
  ...
  const TradeDialog = () => (
    <DialogContent className="sm:max-w-[400px] rounded-[2.5rem] p-6 border-capy-tan/10 shadow-2xl shadow-black/10">
      <div className="flex items-center gap-4 mb-6">
        <div className="w-14 h-14 rounded-2xl overflow-hidden border-2 border-capy-tan/10 shadow-xl bg-white p-1">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 mt-1">
            ...
            <Badge variant="outline" className="text-[8px] h-4 px-1 opacity-40 rounded-md uppercase font-black">{token.dexId}</Badge>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-5">
        <div className="space-y-2">
          ...
          <div className="flex items-center bg-capy-tan/5 rounded-[1.25rem] border border-capy-tan/10 px-4 h-14 transition-all focus-within:border-capy-brown focus-within:ring-4 focus-within:ring-capy-brown/5 group">
            <input 
        ...
        <div className="grid grid-cols-4 gap-2">
          {[0.1, 0.5, 1.0, 5.0].map((amt) => (
            <Button 
              key={amt} 
              variant="outline" 
              className="h-9 text-[11px] font-black rounded-xl border-capy-tan/10 hover:border-capy-brown hover:text-white transition-all active:scale-95"
        ...
        <div className="p-4 bg-capy-tan/5 rounded-[1.25rem] border border-capy-tan/10 space-y-2.5">
        ...
        <div className="grid grid-cols-2 gap-3 pt-1">
          <Button className="h-14 bg-capy-dark hover:bg-capy-brown text-white font-black text-sm gap-2 rounded-[1.25rem] shadow-xl shadow-black/5 transition-all active:scale-95 group">
            <ArrowUpRight size={18} className="group-hover:translate-x-0.5 group-hover:-translate-y-0.5 transition-transform" /> BUY
          </Button>
          <Button variant="outline" className="h-14 border-2 font-black text-sm gap-2 rounded-[1.25rem] hover:bg-rose-50 hover:text-rose-600 hover:border-rose-200 transition-all active:scale-95 group">
      ...
      <Dialog>
        <motion.div layout initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
          <DialogTrigger asChild>
            <Card className="p-2 px-4 flex items-center justify-between gap-4 hover:border-capy-brown/20 hover:shadow-xl hover:shadow-black/[0.02] cursor-pointer bg-white rounded-2xl shadow-none border-capy-tan/10 group transition-all">
              <div className="flex items-center gap-3 min-w-[180px]">
                <div className="relative w-9 h-9 rounded-xl overflow-hidden bg-capy-tan/5 border border-capy-tan/10 flex items-center justify-center font-bold text-[10px] text-capy-brown shrink-0 group-hover:scale-105 transition-transform">
                ...
                <div className="truncate">
                  ...
                  <div className="flex items-center gap-2 mt-0.5">
                    <PriceChange val={token.priceChange?.h24} className="h-4 leading-none" />
              ...
              <div className="hidden lg:grid grid-cols-4 gap-8 flex-1 text-center items-center">
                <div>
                  ...
                  <p className="text-xs font-black text-capy-brown">{formatUSD(token.liquidity?.usd)}</p>
                </div>
                <div>
                  <p className="text-[8px] font-black opacity-30 uppercase tracking-tighter mb-0.5">FDV</p>
                  <p className="text-xs font-black text-capy-dark">{formatUSD(token.fdv)}</p>
                </div>
                <div>
              ...
              <div className="flex items-center gap-2" onClick={(e) => e.stopPropagation()}>
                <DialogTrigger asChild>
                  <Button size="sm" className="h-8 px-4 text-[10px] font-black bg-capy-dark hover:bg-capy-brown rounded-xl shadow-sm transition-all active:scale-95">TRADE</Button>
                </DialogTrigger>
                <Button variant="ghost" size="icon" asChild className="h-8 w-8 rounded-xl border border-capy-tan/10 transition-all opacity-40 hover:opacity-100">
  ...
  return (
    <Dialog>
      <motion.div layout initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.98 }}>
        <DialogTrigger asChild>
          <Card 
            className="h-full flex flex-col overflow-hidden hover:border-capy-brown/30 hover:shadow-[0_20px_50px_rgba(0,0,0,0.08)] cursor-pointer group transition-all duration-500 bg-white rounded-[2rem] shadow-none border-capy-tan/10 relative"
          >
            <div 
            ...
            <div className="relative p-5 flex flex-col h-full">
              <div className="flex justify-between items-start mb-6">
                <div className="relative w-20 h-20 rounded-[1.75rem] overflow-hidden bg-white border-4 border-white shadow-[0_8px_30px_-5px_rgba(0,0,0,0.12)] group-hover:scale-110 group-hover:rotate-2 transition-all duration-500">
                  {token.info?.imageUrl ? (
                    <>
              ...
              <div className="mb-6">
                <div className="flex items-center gap-2">
                  <CardTitle className="text-lg font-black group-hover:text-capy-brown transition-colors truncate">{token.baseToken.symbol}</CardTitle>
              ...
              <div className="grid grid-cols-2 gap-4 mb-6 mt-auto">
                <div className="space-y-0.5">
                  <p className="text-[8px] font-black opacity-20 uppercase tracking-widest leading-none">Liquidity</p>
                  <p className="text-sm font-black text-capy-brown">{formatUSD(token.liquidity?.usd)}</p>
                </div>
                <div className="space-y-0.5 text-right">
                  <p className="text-[8px] font-black opacity-20 uppercase tracking-widest leading-none">Market Cap</p>
              ...
              <div className="flex items-center justify-between gap-2.5 pt-5 border-t border-capy-tan/5">
                <DialogTrigger asChild>
                  <Button className="flex-1 h-10 bg-capy-dark hover:bg-capy-brown text-white text-[10px] font-black uppercase tracking-widest rounded-2xl shadow-xl shadow-black/5 transition-all active:scale-95">
                ...
                <Button 
                  variant="ghost" 
                  size="icon" 
                  className="h-10 w-10 rounded-2xl border border-capy-tan/10 bg-white/50 hover:bg-white transition-all opacity-40 hover:opacity-100"
                  asChild
